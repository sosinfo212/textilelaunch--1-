...
router.post('/login', async (req, res) => {
  try {
    ...
    // Set token in HTTP-only cookie (session ID instead of full token)
    // For localhost development, cookies work across ports on same domain
    // Important: Don't set domain for localhost - let browser handle it
    const cookieOptions = {
      httpOnly: true,
      maxAge: 7 * 24 * 60 * 60 * 1000, // 7 days
      path: '/', // Ensure cookie is available for all paths
      sameSite: 'lax' // 'lax' works for cross-port requests on localhost
    };
